...
COMMENT
ON COLUMN retrieval_index_chunk.source_kind IS
    'First-class upstream source family used to disambiguate table-local ids.';
COMMENT
ON COLUMN retrieval_index_chunk.citation_replay_after IS
    'Typed retry eligibility timestamp for citation metadata replay.';
COMMENT
ON COLUMN retrieval_index_chunk.citation_replay_attempts IS
    'Number of failed citation metadata replay attempts.';
COMMENT
ON COLUMN retrieval_index_chunk.citation_replay_claimed_until IS
    'Lease expiry for multi-instance citation metadata replay claims.';
COMMENT
ON COLUMN retrieval_index_chunk.migration_status IS
