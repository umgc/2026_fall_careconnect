...
SET created_by = c.user_id FROM caregiver c
WHERE ce.caregiver_id = c.id AND ce.created_by IS NULL;
...
SET created_by = ir.created_by FROM incident_reports ir
WHERE ia.incident_report_id = ir.id AND ia.created_by IS NULL;
ALTER TABLE incident_actions
    ALTER COLUMN created_by SET NOT NULL;
DO
$$
