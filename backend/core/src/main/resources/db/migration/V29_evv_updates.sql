...
ALTER TABLE patient
    ADD CONSTRAINT unique_patient_ma_number UNIQUE (ma_number);

-- Add comment for documentation
COMMENT
ON COLUMN patient.ma_number IS 'Medical Assistance Number for EVV compliance';
...
ALTER TABLE evv_record
    ADD CONSTRAINT fk_evv_record_patient
...
-- Add comment
COMMENT
ON COLUMN evv_record.patient_id IS 'Direct reference to patient receiving care - replaces participant_id';
