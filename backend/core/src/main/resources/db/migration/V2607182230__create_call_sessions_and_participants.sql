...
(
    ...
    created_by_user_id BIGINT NOT NULL REFERENCES users
(
    id
),
    scheduled_visit_id BIGINT NULL,
    chime_meeting_id VARCHAR
(
    255
) NULL,
    status VARCHAR
...
(
    ...
    CONSTRAINT uq_call_sessions_call_id UNIQUE
(
    call_id
)
    );
...
(
    ...
    invited_by_user_id BIGINT NULL REFERENCES users
(
    id
),
    status VARCHAR
...
(
    ...
    CONSTRAINT uq_call_participants_session_user UNIQUE
(
    call_session_id,
    user_id
)
