...
COMMENT
ON COLUMN usps_mailpiece.importance_level IS
    'Importance tier: HIGH, MODERATE, LOW, or UNKNOWN (Task 3.14.6).';

COMMENT
ON COLUMN usps_mailpiece.importance_confidence IS
    'Classifier confidence in [0.00, 1.00].';

COMMENT
ON COLUMN usps_mailpiece.classification_method IS
    'How the tier was chosen: RULES, AI, or HYBRID.';

COMMENT
ON COLUMN usps_mailpiece.classification_engine IS
    'Engine id, e.g. rules:v1 or aws_bedrock:amazon.nova-lite-v1:0.';

COMMENT
ON COLUMN usps_mailpiece.importance_reasoning IS
    'Human-readable recorded rationale for the classification decision.';

COMMENT
ON COLUMN usps_mailpiece.importance_category IS
