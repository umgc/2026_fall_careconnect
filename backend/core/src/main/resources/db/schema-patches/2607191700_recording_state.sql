...
SET owner_user_id    = initiated_by_user_id,
    purpose          = CASE
                           WHEN initiated_by_user_id IS NULL
                               THEN 'SYSTEM_TRANSCRIPTION'
                           ELSE 'USER_PLAYBACK' END,
...
WITH ranked AS (SELECT id, ROW_NUMBER() OVER (PARTITION BY call_id ORDER BY started_at, id) AS generation
                FROM call_recordings)
UPDATE call_recordings recording
SET generation = ranked.generation FROM ranked
...
(
    ...
    attempt_count INTEGER NOT NULL DEFAULT 0,
    next_attempt_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    aws_job_name VARCHAR
(
    200
) NOT NULL,
    media_bucket VARCHAR
(
    255
) NOT NULL,
    media_key VARCHAR
...
(
    ...
    updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    completed_at TIMESTAMPTZ,
    CONSTRAINT uq_post_call_transcription_recording UNIQUE
(
    recording_id
),
