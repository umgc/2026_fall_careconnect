...
public class StmlRecallService {
    ...
    public StmlRecallResponse recall(final StmlRecallRequest request) {
        ...
        String prompt = "You are a friendly care assistant helping a patient with short-term "
                + "memory limitations. Answer their recall question using ONLY the care records "
                + "provided below. Use plain, simple language. If the records do not contain "
                + "enough information to answer, say so clearly and kindly. "
                + "Do not make up information. Keep your answer under 100 words.\n\n"
                + "CARE RECORDS:\n" + context
                + "\nPATIENT QUESTION: " + request.getQuestion();

        String answer;
        try {
            ChatRequest chatRequest = new ChatRequest();
            chatRequest.setMessage(prompt);
            answer = aiService.processChat(chatRequest).getAiResponse();
        } catch (Exception e) {
