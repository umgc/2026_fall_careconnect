...
function PatientProfilePage({
  ...
  const shareProfileNative = async () => {
    if (!profileShare) return;
    const shareText = `View my CareConnect profile: ${profileShare.url}`;
                ...
                <div className="flex flex-col gap-3 items-center">
                  ...
                  <div className="flex flex-col gap-2 w-full">
                    <OptionDropdown
                      ...
                      options={[
                        { value: "sms" as const, label: "SMS / Text message", description: "Open your messaging app with the link" },
                        { value: "email" as const, label: "Email", description: "Open email with the profile link" },
                        { value: "whatsapp" as const, label: "WhatsApp", description: "Share the link in WhatsApp" },
                      ]}
                    />
                    <button type="button" onClick={shareProfileNative}
                      ...
                      style={{ background: theme.color }}>
                      <Send size={16} />
                      {shareChannel === "sms" && "Share via SMS"}
                      {shareChannel === "email" && "Share via Email"}
                      {shareChannel === "whatsapp" && "Share via WhatsApp"}
                    </button>
