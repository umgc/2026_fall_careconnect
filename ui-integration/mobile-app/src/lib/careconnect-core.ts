...
export function buildCaregiverPatientRoster(opts: {
  ...
  const byEmail = opts.caregiverEmail?.trim()
    ? opts.linkedCaregivers.find(
        c => !!c.email?.trim() && namesMatch(c.email!, opts.caregiverEmail!),
      )
    : undefined;
  // Prefer a single active Care Circle member with grants so patient-granted
  // access shows immediately even when caregiver ids differ across sessions.
  const activeWithGrants = opts.linkedCaregivers.filter(
    c => c.status === "active" && c.grants.length > 0,
  );
  const bySoloActive =
    !byInvite && !byId && !byName && !byEmail && activeWithGrants.length === 1
      ? activeWithGrants[0]
      : undefined;
  const activeOrPending = opts.linkedCaregivers.filter(c => c.status !== "suspended");
  const bySolo =
    !byInvite && !byId && !byName && !byEmail && !bySoloActive && activeOrPending.length === 1
      ? activeOrPending[0]
      : undefined;
  const link = byInvite || byId || byName || byEmail || bySoloActive || bySolo;
